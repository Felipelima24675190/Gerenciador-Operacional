...
export default function ViewTransitScreen({ multas, motoristas }: ViewTransitScreenProps) {
  ...
  if (multas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center p-8 bg-white rounded-card border border-gray-200 shadow-card">
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-card border border-gray-200 shadow-card p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label htmlFor="transit-filial" className="text-2xs font-bold text-slate-500 uppercase tracking-wide block mb-1">Filial</label>
            <select
              id="transit-filial"
              value={filialFilter}
          ...
          <div>
            <label htmlFor="transit-data-inicio" className="text-2xs font-bold text-slate-500 uppercase tracking-wide block mb-1">Data Início</label>
            <div className="relative">
              <Calendar size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
              <input
                id="transit-data-inicio"
                type="date"
                value={dataInicio}
                onChange={e => setDataInicio(e.target.value)}
                aria-label="Data de início do filtro"
                className="w-full px-3 py-2 pl-8 text-xs font-medium border border-gray-200 rounded-button bg-slate-50 focus:ring-2 focus:ring-brand-400 focus:outline-none"
              />
            </div>
          </div>
          <div>
            <label htmlFor="transit-data-fim" className="text-2xs font-bold text-slate-500 uppercase tracking-wide block mb-1">Data Fim</label>
            <div className="relative">
              <Calendar size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
              <input
                id="transit-data-fim"
          ...
          <div className="flex items-end">
            <p className="text-2xs text-slate-500 font-bold">{filteredMultas.length} registros</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Top Motoristas */}
        <div className="bg-white rounded-card border border-gray-200 shadow-card p-4">
          <h4 className="text-2xs font-black text-slate-600 uppercase tracking-wide mb-3">Top Motoristas com Mais Multas</h4>
          {topMotoristas.length > 0 ? (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topMotoristas} layout="vertical" margin={{ left: 8 }}>
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} width={90} interval={0} axisLine={false} />
                  <Tooltip
        ...
        {/* Top Filiais */}
        <div className="bg-white rounded-card border border-gray-200 shadow-card p-4">
          <h4 className="text-2xs font-black text-slate-600 uppercase tracking-wide mb-3">Filiais com Mais Multas</h4>
          {topFiliais.length > 0 ? (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topFiliais} layout="vertical" margin={{ left: 8 }}>
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} width={55} interval={0} axisLine={false} />
        ...
        <div className="bg-white rounded-card border border-gray-200 shadow-card p-4">
          <h4 className="text-2xs font-black text-slate-600 uppercase tracking-wide mb-3">Órgãos Atuadores</h4>
          {topOrgaos.length > 0 ? (
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white rounded-card border border-gray-200 shadow-card overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <input
            ...
            aria-label="Buscar na tabela de multas de trânsito"
          ...
          <table className="w-full text-xs text-left">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-800 text-white text-2xs uppercase tracking-wide">
                ...
                <th className="px-3 py-2.5">Órgão</th>
                <th className="px-3 py-2.5 max-w-[200px]">Infração</th>
                <th className="px-3 py-2.5 whitespace-nowrap">Auto</th>
                <th className="px-3 py-2.5 whitespace-nowrap">Vl. Cobrado</th>
                <th className="px-3 py-2.5 whitespace-nowrap">Vl. Recuperado</th>
                <th className="px-3 py-2.5">Globus</th>
                    ...
                    <td className="px-3 py-2 min-w-[120px]">
                      {m.motoristaIdentificado ? (
                        <>
                          ...
                          <p className="text-2xs text-slate-400">{m.matriculaMotorista}</p>
                          {!mot && <p className="text-2xs text-amber-600 italic">Desligado</p>}
                        </>
                      ) : (
